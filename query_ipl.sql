...
SELECT count(R) AS `Innings`,
       sum(R) AS `Runs`,
       sum(B) AS `Balls`,
       sum(dismissal LIKE 'not%') AS 'Not out',
       max(R) AS `Highest`,
       cast((sum(R) / (count(R) - sum(dismissal LIKE 'not%'))) AS DECIMAL(5,2)) `Average`,
...
FROM
  (SELECT SUBSTRING_INDEX(batsman, '(', 1) AS `batsman`,
   ...
   GROUP BY `batsman`,
            `team`) tb1
GROUP BY `Player` HAVING `Average`>25
ORDER BY `SR` DESC LIMIT 25 
...
#Captains batting records aggregated
SELECT count(R) AS `Innings`,
       sum(R) AS `Runs`,
       sum(B) AS `Balls`,
       sum(dismissal LIKE 'not%') AS 'Not out',
...
WHERE batsman LIKE '%(captain)%' #Most runs BY a captain
  SELECT batsman, AS `Player`,
         ...
         sum(M) AS `Minutes`,
         team AS `Team`
  FROM ipl_batting WHERE batsman LIKE '%(captain)%'
GROUP BY batsman,
         team
ORDER BY `Runs` DESC LIMIT 25 INTO outfile '/var/lib/mysql-files/export/ipl/Most_runs_captain_ipl.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';
