...
overall_runs_by_team_bar <- ggplot(Overall_runs_by_teams_ipl,
  ...
  geom_bar(position = "dodge", stat = "identity") + ggtitle("Total Runs scored by the Teams\n") + xlab("") + ylab("Runs") +
  coord_flip() + theme_new() +
  scale_fill_manual(values = wes_palette("FantasticFox"))+
  theme(plot.title=element_text(size=rel(1.5), hjust=0.25,
                                face = "bold",
                                colour = "#000c52"))

ggsave("graphs/overall_runs_by_team_bar.svg", overall_runs_by_team_bar, 
       width = 13, height = 8) 

#ggplotly(overall_runs_by_team_gg, tooltip = c("text", "y", "fill"))

overall_runs_by_team_pie <- Overall_runs_by_teams_ipl %>% filter(Result == "All") %>%
  ggplot(., aes(x = Result, y = Runs, fill = Team)) +
  geom_bar(stat = "identity") + ggtitle("Runs scored by the Teams") +
  theme_new() + coord_polar(theta = "y", direction = -1) + 
  theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
        legend.position = "right", 
        legend.direction = "vertical", legend.key.size = unit(2, 'lines') , axis.title = element_blank()) +
  geom_text(aes(x=1.2, y = Runs/4  + c(0, cumsum(Runs)[-length(Runs)]),
                label = round(Runs * 100 / sum(Runs), digits = 1)), 
            size = 7) + 
  scale_fill_economist()

ggsave("graphs/overall_runs_by_team_pie.svg", overall_runs_by_team_pie,
       width = 13, height = 8) 
...
ggsave("graphs/all_total_ipl_bar_new.svg", 
       all_total_ipl_bar)

ggiraph(code = print(all_aggregate_ipl_bar), width_svg = 26,
        height_svg = 6)
