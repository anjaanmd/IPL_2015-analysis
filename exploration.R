...
overall_runs_distribution_all_pie <-
  ...
  geom_text(aes(y = Runs / 2 + c(0, cumsum(Runs)[-length(Runs)]),
                label = percent(Runs / sum(Runs)))) +
  theme(plot.title = element_text(size = 15, margin = margin(b = -5, unit = "pt")))
...
ggplot(overall_runs_distribution,
       aes(x = Result, y = Runs, fill = Runs_class)) +
  geom_bar(stat = "identity", width = 1) + ggtitle("Total Runs scored by the Teams") + xlab("") + ylab("Runs") +
  coord_flip() + theme_mine()
...
ggplot(overall_extras_all,
       aes(x = Result, y = Runs, fill = Extras_class)) +
  geom_bar(stat = "identity", width = 1) + ggtitle("Total Runs scored by the Teams") + xlab("") + ylab("Runs") +
  theme_mine() + coord_polar(theta = "y") + theme(axis.text.x = element_blank()) +
  geom_text(aes(y = Runs / 2 + c(0, cumsum(Runs)[-length(Runs)]),
                label = percent(Runs / sum(Runs))))
